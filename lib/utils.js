...
const self = module.exports = {
  ...
  /**
   *  Logs the bot into Facebook.
   *
   *  @param    {Object}   credentials  An object containing the facebook credentials for the bot
   *  @returns  {Promise}  Resolves to a chat object with messenger API methods
   */
  login: credentials =>
    new Promise((resolve, reject) => {
      chatApi(credentials, (err, chat) => {
        if (err) {
          reject(err);
        } else {
          _.assign(chat, { sendMsg: (msg, toId) => {
            let recipient = toId;
            if (ENV === 'development') {
          ...
          setTimeout(() => self.checkPresence(chat), 60000);

          self.writeAppState(chat.getAppState());
  ...
  writeAppState: appState =>
    _.attempt(() =>
      fs.writeFileSync(APP_STATE_PATH, self.prettyPrint(appState), 'utf8')
    ),
  /**
  ...
  assignEventProps: (event) => {
    const props = {
      ...
      subCmd: self.getSubCmd(event),
      attachv: _.toArray(event.attachments),
      toId: ENV !== 'development'
        ? event.threadID
  ...
  logEvent: (event) => {
    self.debug(event);
    if (self.isntBot) {
      let msg = `[${event.threadID}] ${event.senderName}: `;
      if (event.body) {
        msg += event.body;
      } else {
  ...
  isSaveArtEvent: (writeLock, event) =>
    artQueue.indexOf(event.senderID) > -1
    && !writeLock && event.attachv[0]
    && event.attachv[0].previewUrl,
  getWordsv: ({ body = '' }) =>
    _.words(body),
  ...
  getSubCmd: (event) => {
    ...
    const result = _.attempt(() => {
      const str = event.body.split(`/${event.cmd}`)[1];
      let subCmd;

      if (event.cmd === 'countdown') {
        subCmd = _.camelCase(str);
      } else if (event.cmd === 'yoda') {
        subCmd = str.trim();
      } else {
   ...
   *
   *  @param   {Object}  event  An options object
   *  @returns {void}
   */
   ...
   *
   *  @param    {Object}  event The facebook event to check for possible auto
   *  responses
...
/**
 *  Gets the Base URL for a FanDuel API call
 *
 *  @param    {Object}  event  The Facebook event
 ...
 */
  ...
  /**
   *  The rank string to use in the FanDuel scoreboard
   *
   *  @param    {Number}  rank  The scoreboard rank
  ...
  checkPresence: (chat) => {
    ...
    chat.getThreadInfo('1184034474942360', (getThreadErr, info) => {
      if (getThreadErr) {
        ...
        if (info.nicknames[userIds.bot] !== 'Bot') {
          chat.changeNickname('Bot', '1184034474942360', userIds.bot);
        }
  ...
  getArtFiles: () =>
    artFiles,
  addArtQueue: (event) => {
    artQueue.push(event.senderId);
  },
  rmArtQueue: event =>
  artQueue.splice(artQueue.indexOf(event.senderID), 1),
  getArtQueue: () =>
    artQueue,
  isEmoji: event =>
    event.body && event.body.length <= 2 && emojiRegex().test(event.body),
};

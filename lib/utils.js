...
const self = module.exports = {
  ...
  getSubCmd: (cmd, event) => {
    const result = _.attempt(() => {
      const subCmd = cmd !== 'countdown'
        ? _.lowerCase(_.words(event.body)[1])
        : _.camelCase(event.body.split('/countdown')[1]);
  ...
  addAutoResponseMatch: ({ matches, autoResv, body, type }) => {
    if (body) {
      ...
      autoResv.forEach((str) => {
        self.debug(`checking for ${str} in ${body}`);
        if (body.match(new RegExp(`\\b${str}\\b`))) {
